...
Aravo_data <- as.matrix(aravo$env[,-c(3,5)])
true_lab_Aravo <- as.numeric(aravo$env$ZoogD)   
...
model {
  for (i in 1:P) {
   ...
   for (g in 1:G) {
    for (j in 1:D) {
       mu_g[j, g] ~ dgamma(4.6802774861,13.86219672)        
       lambda_g[j, g] ~ dgamma(16.037965,3.049896396) 
    }
     alpha[g] ~ dgamma(15,5.0508720)   
   }
...
#--------------------------------------------#
fviz_cluster(list(data = Aravo_scaled_data, cluster = z_mode_Aravo), geom = "point", ellipse.type = "convex", ggtheme = theme_minimal(), main = "True label Clustering")
...
table( true_lab_Aravo , true_lab_Aravo)
...
# Call the function
#--------------------#
dp_Gmvlg_metricsAravo_1 <- calculate_dp_Gmvlg_clustering_metrics(data_1 = Aravo_scaled_data, true_clusters = true_lab_Aravo, z_mode = z_mode_Aravo)
...
k <- length(unique(true_lab_Aravo))  
...
for (i in seq_along(gini_values)) {
  ...
  tic("Genie Runtime")
  res <- try(gclust(Aravo_scaled_data, gini_threshold = gini), silent = TRUE)
  run_time_Aravo <- toc()
  ...
  if (inherits(res, "try-error") || is.null(res)) {
    ...
    # Use your relabeling function here
    relabeled <- relabel_clusters_optimal(cluster_labels, true_lab_Aravo)
    ...
    kappa_result <- kappa2(data.frame(rater1 = true_lab_Aravo, rater2 = aligned_labels))
...
#-------------------------------------------------------#
tic("Genie Runtime")
res <- gclust(Aravo_scaled_data, gini_threshold = 0.401)
run_time_Aravo_Genie <- toc()

k <- length(unique(true_lab_Aravo))

aligned_labels <- cutree(res, k = k)

table( true_lab_Aravo , aligned_labels)
kappa2(data.frame(rater1 = true_lab_Aravo, rater2 = aligned_labels))
...
calculate_Genie_clustering_metrics <- function(data_1, true_clusters, cluster_labels) {
  ...
  results_list[["Genie"]] <- list(
    ...
    ARI = ari_result,
    NMI = nmi_result,
    CPU_RUN_TIME = run_time_Aravo_Genie$callback_msg  # Store the runtime
  )
  return(results_list)
}
...
# Call the function
#------------------#
Genie_metricsAravo_1 <- calculate_Genie_clustering_metrics(data_1 = Aravo_scaled_data, true_clusters = true_lab_Aravo, cluster_labels = aligned_labels)

#------------------#
#     Results 
#------------------#
for (Genie in names(Genie_metricsAravo_1)) {
  cat("\n", Genie, " Clustering Results:\n", sep="")
  cat("Table:\n")
  print(Genie_metricsAravo_1[[Genie]]$Table)
  cat("Kappa:", Genie_metricsAravo_1[[Genie]]$Kappa, "\n")
  cat("Adjusted Rand Index:", Genie_metricsAravo_1[[Genie]]$ARI, "\n")
  cat("NMI:", Genie_metricsAravo_1[[Genie]]$NMI, "\n")
  cat(" CPU RUN TIME:", Genie_metricsAravo_1[[Genie]]$CPU_RUN_TIME, "\n")
}

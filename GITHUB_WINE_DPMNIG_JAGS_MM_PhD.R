...
x <- wine[,-1] # Convert all columns to numeric
x <- scale(x)
true_lab <- wine$Type    # Use the 'class' column as the true labels

# Load the Corvus dataset
#data("mollusc", package ="ade4")
#x <- mollusc$fau
#x <- scale(x)
#true_lab=mollusc$plan$method

# Load the Sonar dataset
#data("Sonar")
#x <- Sonar[,-c(16,17,20,48,61)] # Convert all columns to numeric
#x <- scale(x)
#true_lab <- Sonar$Class    # Use the 'class' column as the true labels
...
for(chain in 1:2){
  ...
  while (it<(max_it+1)){
    ...
    for (i in 1:n){
      ...
      prob_g <- ifelse(is.na(prob_g), 1e-10, prob_g)
...
convchains <- c(1,2)[which(chain_conv_ind==1)]
# convchains <- c(1,2,3)[which(all_final_G==4)]
if(length(convchains)==0){
  endlik <- c(all_loglik[[1]][it-1],all_loglik[[2]][it-1])
...
Predicted_Labels = ord_final_class
###################################
##### Save all results ############
###################################
...
Predicted_Labels
Predicted_Labels1 <- as.numeric(Predicted_Labels)
Predicted_Labels1
DPMNIG_time <- toc()
true_lab <- as.numeric(true_lab)
true_lab
table(true_lab, Predicted_Labels1)
new <- c(1,2)
old <- c(2,1)
Predicted_Labels1[Predicted_Labels1 %in% old] <- new[match(Predicted_Labels1,old,nomatch = 0)]
table(true_lab, Predicted_Labels1)
kappa2(data.frame(rater1 = true_lab, rater2 = Predicted_Labels1))

fviz_cluster(list(data = x, cluster = Predicted_Labels1), geom = "point", ellipse.type = "convex", ggtheme = theme_minimal(), main = "DPMNIG Clustering")

calculate_dp_mnig_clustering_metrics <- function(data, true_clusters) {
  ...
  table_result <- table(true_clusters, Predicted_Labels1)
  kappa_result <- kappa2(data.frame(rater1 = true_clusters, rater2 = Predicted_Labels1))
  ari_result <- adjustedRandIndex(true_clusters, Predicted_Labels1)
  nmi_result <- NMI(true_clusters, Predicted_Labels1)
  
  # Store results
  results_list[["dp_mnig"]] <- list(
    Table = table_result,
    Kappa = kappa_result$value,
    ARI = ari_result,
    NMI = nmi_result,
    CPU_RUN_TIME = DPMNIG_time$callback_msg  # Store the runtime
  )
...
# Call the function
dp_mnig_metrics <- calculate_dp_mnig_clustering_metrics(data = x, true_clusters = true_lab)

# Print the results for each method
for (dp_mnig in names(dp_mnig_metrics)) {
  cat("\n", dp_mnig, " Clustering Results:\n", sep="")
  cat("Table:\n")
  print(dp_mnig_metrics[[dp_mnig]]$Table)
  cat("Kappa:", dp_mnig_metrics[[dp_mnig]]$Kappa, "\n")
  cat("Adjusted Rand Index:", dp_mnig_metrics[[dp_mnig]]$ARI, "\n")
  cat("NMI:", dp_mnig_metrics[[dp_mnig]]$NMI, "\n")
  cat(" CPU RUN TIME:", dp_mnig_metrics[[dp_mnig]]$CPU_RUN_TIME, "\n")
}

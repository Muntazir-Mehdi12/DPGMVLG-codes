...
model {
  for (i in 1:P) {
   ...
   for (g in 1:G) {
    for (j in 1:D) {
       mu_g[j, g] ~ dgamma(5.03383531,1.86997635)   
       lambda_g[j, g] ~ dgamma(3.640603976,0.123054052)
    }
     alpha[g] ~ dbeta(11.9,9.1) 
   }
...
#--------------------------------------------#
fviz_cluster(list(data = Meau_scaled_data, cluster = z_mode_Meau), geom = "point", ellipse.type = "convex", ggtheme = theme_minimal(), main = "True label Clustering")
...
# relabeling function
relabeled <- relabel_clusters_optimal(z_mode_Meau, true_lab_Meau)
z_mode_Meau <- relabeled$remapped_labels

cat("  Confusion matrix (after relabeling):\n")
print(relabeled$confusion)

# Cohen's Kappa
kappa_result <- kappa2(data.frame(rater1 = true_lab_Meau, rater2 = z_mode_Meau))
...
table(true_lab_Meau , true_lab_Meau)
table(true_lab_Meau , z_mode_Meau)
...
calculate_dp_Gmvlg_clustering_metrics <- function(data_1, true_clusters, z_mode) {
  ...
  results_list[["dp_Gmvlg"]] <- list(
    ...
    ARI = ari_result,
    NMI = nmi_result,
    CPU_RUN_TIME = run_time_Meau$callback_msg 
  )
...
# Call the function
#--------------------#
dp_Gmvlg_metricsMeau_1 <- calculate_dp_Gmvlg_clustering_metrics(data_1 = Meau_scaled_data, true_clusters = true_lab_Meau, z_mode = z_mode_Meau)
...
# relabeling function
relabeled_kmeans <- relabel_clusters_optimal(kmeans_clusters_Meau, true_lab_Meau)
kmeans_clusters_Meau <- relabeled_kmeans$remapped_labels
...
for (i in seq_along(gini_values)) {
  ...
  tic("Genie Runtime")
  res <- try(gclust(Meau_scaled_data, gini_threshold = gini), silent = TRUE)
  run_time_Meau <- toc()
  ...
  if (inherits(res, "try-error") || is.null(res)) {
    ...
    relabeled <- relabel_clusters_optimal(cluster_labels, true_lab_Meau)
    aligned_labels <- relabeled$remapped_labels
    
    cat("  Confusion matrix (after relabeling):\n")
    print(relabeled$confusion)
    
    kappa_result <- kappa2(data.frame(rater1 = true_lab_Meau, rater2 = aligned_labels))
...
#-------------------------------------------------------#
tic("Genie Runtime")
res <- gclust(Meau_scaled_data, gini_threshold = 0.001)
run_time_Meau_Genie <- toc()

k <- length(unique(true_lab_Meau))

aligned_labels <- cutree(res, k = k)

table( true_lab_Meau , aligned_labels)
kappa2(data.frame(rater1 = true_lab_Meau, rater2 = aligned_labels))
...
calculate_Genie_clustering_metrics <- function(data_1, true_clusters, cluster_labels) {
  ...
  results_list[["Genie"]] <- list(
    ...
    ARI = ari_result,
    NMI = nmi_result,
    CPU_RUN_TIME = run_time_Meau_Genie$callback_msg  # Store the runtime
  )
...
# Call the function
#------------------#
Genie_metricsMeau_1 <- calculate_Genie_clustering_metrics(data_1 = Meau_scaled_data, true_clusters = true_lab_Meau, cluster_labels = aligned_labels)

#------------------#
#     Results 
#------------------#
for (Genie in names(Genie_metricsMeau_1)) {
  cat("\n", Genie, " Clustering Results:\n", sep="")
  cat("Table:\n")
  print(Genie_metricsMeau_1[[Genie]]$Table)
  cat("Kappa:", Genie_metricsMeau_1[[Genie]]$Kappa, "\n")
  cat("Adjusted Rand Index:", Genie_metricsMeau_1[[Genie]]$ARI, "\n")
  cat("NMI:", Genie_metricsMeau_1[[Genie]]$NMI, "\n")
  cat(" CPU RUN TIME:", Genie_metricsMeau_1[[Genie]]$CPU_RUN_TIME, "\n")
}

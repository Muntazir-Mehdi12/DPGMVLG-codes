...
#--------------------------------------------#
fviz_cluster(list(data = Corvus_scaled_data, cluster = z_mode_Corvus), geom = "point", ellipse.type = "convex", ggtheme = theme_minimal(), main = "True label Clustering")
...
# Call the function
#--------------------#
dp_Gmvlg_metricsCorvus_1 <- calculate_dp_Gmvlg_clustering_metrics(data_1 = Corvus_scaled_data, true_clusters = true_lab_Corvus, z_mode = z_mode_Corvus)
...
k <- length(unique(true_lab_Corvus))
...
for (i in seq_along(gini_values)) {
  ...
  tic("Genie Runtime")
  res <- try(gclust(Corvus_scaled_data, gini_threshold = gini), silent = TRUE)
  run_time_Corvus <- toc()
  ...
  if (inherits(res, "try-error") || is.null(res)) {
    ...
    relabeled <- relabel_clusters_optimal(cluster_labels, true_lab_Corvus)
    aligned_labels <- relabeled$remapped_labels
    
    cat("  Confusion matrix (after relabeling):\n")
    print(relabeled$confusion)

    kappa_result <- kappa2(data.frame(rater1 = true_lab_Corvus, rater2 = aligned_labels))
...
#-------------------------------------------------------#
tic("Genie Runtime")
res <- gclust(Corvus_scaled_data, gini_threshold = 0.001)
run_time_Corvus_Genie <- toc()

k <- length(unique(true_lab_Corvus))

aligned_labels <- cutree(res, k = k)

table( true_lab_Corvus , aligned_labels)
kappa2(data.frame(rater1 = true_lab_Corvus, rater2 = aligned_labels))
...
calculate_Genie_clustering_metrics <- function(data_1, true_clusters, cluster_labels) {
  ...
  results_list[["Genie"]] <- list(
    ...
    ARI = ari_result,
    NMI = nmi_result,
    CPU_RUN_TIME = run_time_Corvus_Genie$callback_msg  # Store the runtime
  )
  return(results_list)
}
...
# Call the function
#------------------#
Genie_metricsCorvus_1 <- calculate_Genie_clustering_metrics(data_1 = Corvus_scaled_data, true_clusters = true_lab_Corvus, cluster_labels = aligned_labels)

#------------------#
#     Results 
#------------------#
for (Genie in names(Genie_metricsCorvus_1)) {
  cat("\n", Genie, " Clustering Results:\n", sep="")
  cat("Table:\n")
  print(Genie_metricsCorvus_1[[Genie]]$Table)
  cat("Kappa:", Genie_metricsCorvus_1[[Genie]]$Kappa, "\n")
  cat("Adjusted Rand Index:", Genie_metricsCorvus_1[[Genie]]$ARI, "\n")
  cat("NMI:", Genie_metricsCorvus_1[[Genie]]$NMI, "\n")
  cat(" CPU RUN TIME:", Genie_metricsCorvus_1[[Genie]]$CPU_RUN_TIME, "\n")
}

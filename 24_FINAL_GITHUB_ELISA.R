...
#--------------------------------------------#
fviz_cluster(list(data = Elisa_scaled_data, cluster = z_mode_Elisa), geom = "point", ellipse.type = "convex", ggtheme = theme_minimal(), main = "True label Clustering")
...
kappa2(data.frame(rater1 = true_lab_Elisa, rater2 = z_mode_Elisa))
...
# Call the function
#--------------------#
dp_Gmvlg_metricsElisa_1 <- calculate_dp_Gmvlg_clustering_metrics(data_1 = Elisa_scaled_data, true_clusters = true_lab_Elisa, z_mode = z_mode_Elisa)
...
k <- length(unique(true_lab_Elisa))
...
for (i in seq_along(gini_values)) {
  ...
  tic("Genie Runtime")
  res <- try(gclust(Elisa_scaled_data, gini_threshold = gini), silent = TRUE)
  run_time_Elisa <- toc()
  ...
  if (inherits(res, "try-error") || is.null(res)) {
    ...
    relabeled <- relabel_clusters_optimal(cluster_labels, true_lab_Elisa)
    aligned_labels <- relabeled$remapped_labels
    
    cat("  Confusion matrix (after relabeling):\n")
    print(relabeled$confusion)
 
    kappa_result <- kappa2(data.frame(rater1 = true_lab_Elisa, rater2 = aligned_labels))
...
#-------------------------------------------------------#
tic("Genie Runtime")
res <- gclust(Elisa_scaled_data, gini_threshold = 0.001)
...
table( true_lab_Elisa , aligned_labels)
kappa2(data.frame(rater1 = true_lab_Elisa, rater2 = aligned_labels))
...
calculate_Genie_clustering_metrics <- function(data_1, true_clusters, cluster_labels) {
  ...
  results_list[["Genie"]] <- list(
    ...
    ARI = ari_result,
    NMI = nmi_result,
    CPU_RUN_TIME = run_time_Elisa_Genie$callback_msg 
  )
...
# Call the function
#------------------#
Genie_metricsElisa_1 <- calculate_Genie_clustering_metrics(data_1 = Elisa_scaled_data, true_clusters = true_lab_Elisa, cluster_labels = aligned_labels)

#------------------#
#     Results 
#------------------#
for (Genie in names(Genie_metricsElisa_1)) {
  cat("\n", Genie, " Clustering Results:\n", sep="")
  cat("Table:\n")
  print(Genie_metricsElisa_1[[Genie]]$Table)
  cat("Kappa:", Genie_metricsElisa_1[[Genie]]$Kappa, "\n")
  cat("Adjusted Rand Index:", Genie_metricsElisa_1[[Genie]]$ARI, "\n")
  cat("NMI:", Genie_metricsElisa_1[[Genie]]$NMI, "\n")
  cat(" CPU RUN TIME:", Genie_metricsElisa_1[[Genie]]$CPU_RUN_TIME, "\n")
}

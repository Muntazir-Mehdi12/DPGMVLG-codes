...
update_V<-function(a0,a1,a2,a3,a4,x,z,U,Uinv,d){
  ...
  for (g in 1:length(a0)){
    ...
    V <- A-V2-V3-V4-V5
    det_value <- det(V)
    if (is.na(det_value) || det_value < 1e-6) {
      V0[[g]] <- solve(cov(x))
    } else {
      V0[[g]] <- solve(V, tol=set.tol)
    }
  }
  return(V0)
}
...
x <- elisa[, -c(1)]  # Exclude the 'class' column for clustering
true_lab <- elisa$type    # Use the 'class' column as the true labels
